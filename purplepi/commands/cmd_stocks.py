...
def cli(ctx):
    ...
    while True:
        ...
        tickers = ["MSFT", "AAPL", "TSLA", "NFLX", "DIS", "QQQ", "SPY"]
        ...
        for ticker in tickers:
            ...
            last_quote = str((data.tail(1)['Close'].iloc[0]))

            ctx.log(ticker + ": " + last_quote)
            draw.text((x, y), str(ticker), font=font, fill="#0000FF")
            y += font.getsize(ticker)[1]
            draw.text((x, y), str(last_quote), font=font, fill="#FF0000")
            disp.image(image, rotation)
